...
public class DatabaseManager extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String CREATE_GROUP_TABLE = "CREATE TABLE " + TABLE_GROUP + "("
				...
				+ KEY_STATUS + " TEXT, " + KEY_ROUND + " TEXT)";

		String CREATE_IDEA_TABLE = "CREATE TABLE " + TABLE_IDEA + "(" + KEY_ID
				+ " INTEGER PRIMARY KEY, " + KEY_AUTHOR_NAME + " TEXT, "
				+ KEY_PATH + " TEXT, " + FK_IDEA_KEY + " INTEGER, FOREIGN KEY("
				+ FK_IDEA_KEY + ") REFERENCES tbl_group(id))";
	 ...
	 *            Table to update
	 * @param id
	 *            Id of row which we want to update
	...
	public long updateRecord(ContentValues values, String table, int id) {
		...
		SQLiteDatabase db = this.getWritableDatabase();
		numRows = db.update(table, values, KEY_ID + " = ?",
	 ...
	 *            Table name
	 * @param id
	 *            ID of row which we want to delete
	...
	public long deleteRecord(String table, int id) {
		...
		SQLiteDatabase db = this.getWritableDatabase();
		numRows = db.delete(table, KEY_ID + " = ?",
				new String[] { String.valueOf(id) });
		db.close();
