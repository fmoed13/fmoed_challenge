...
public class SessionManager extends Activity {
	 ...
	 * @param strings List of argument to put into URL
	 * @return String URL
	 */
		...
		.setCallback(new FutureCallback<JsonObject>() {
		   @Override
		    public void onCompleted(Exception e, JsonObject result) {
			   ...
			   Integer status = Integer.parseInt(result.get("round").toString());
			   
			   if(status == 0) {
				   Log.i("checkSession", "session not started or finished");
			   } else {
				   Log.i("checkStasion", "session in round " + status.toString());
			   }
		    }
		});
	}
